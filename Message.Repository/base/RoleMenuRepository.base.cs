...
namespace Message.Repository
{
    public sealed partial class RoleMenuRepository : MessageManagementDBRepository<RoleMenu>, IRoleMenuRepository
    {
        ...
        protected override IQueryable<RoleMenu> SelectFilterB(DbContext DB, RoleMenu oSearchEntity, IQueryable<RoleMenu> query, string sOperator = null)
        {
            if (oSearchEntity != null)
            {
                if (oSearchEntity.Id != 0) { query = query.Where(x => x.Id == oSearchEntity.Id); }
        ...
        protected override IQueryable<RoleMenu> OrderBySingleField(IQueryable<RoleMenu> query, string sSortName = null, string sSortOrder = null)
        {
            if (sSortOrder == "desc")
            {
                switch (sSortName)
