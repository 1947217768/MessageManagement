...
{
    public sealed partial class DataTypeRepository : MessageManagementDBRepository<DataType>, IDataTypeRepository
    {
        protected override IQueryable<DataType> SearchFilterB(DbContext DB, DataType oSearchEntity, IQueryable<DataType> query, string sOperator = null)
        {
            if (oSearchEntity != null)
            {
                if (oSearchEntity.Id != 0) { query = query.Where(x => x.Id == oSearchEntity.Id); }
                if (!string.IsNullOrWhiteSpace(oSearchEntity.StypeName)) { query = query.Where(x => x.StypeName.Contains(oSearchEntity.StypeName)); }
        ...
        protected override IQueryable<DataType> SelectFilterB(DbContext DB, DataType oSearchEntity, IQueryable<DataType> query, string sOperator = null)
        {
            if (oSearchEntity != null)
            {
                if (oSearchEntity.Id != 0) { query = query.Where(x => x.Id == oSearchEntity.Id); }
        ...
        protected override IQueryable<DataType> OrderBySingleField(IQueryable<DataType> query, string sSortName = null, string sSortOrder = null)
        {
            if (sSortOrder == "desc")
            {
                switch (sSortName)
                {
                    ...
                    case "StypeName": return query.OrderByDescending(x => x.StypeName).ThenBy(x => x.Id);
            ...
            else
            {
                switch (sSortName)
                {
                    ...
                    case "StypeName": return query.OrderBy(x => x.StypeName).ThenBy(x => x.Id);
