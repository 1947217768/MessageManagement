...
{
    public sealed partial class MemoRepository : MessageManagementDBRepository<Memo>, IMemoRepository
    {
        protected override IQueryable<Memo> SearchFilterB(DbContext DB, Memo oSearchEntity, IQueryable<Memo> query, string sOperator = null)
        {
            if (oSearchEntity != null)
            {
                ...
                if (!string.IsNullOrWhiteSpace(oSearchEntity.Scontent)) { query = query.Where(x => x.Scontent.Contains(oSearchEntity.Scontent)); }
            }
            return query;
        }
        protected override IQueryable<Memo> SelectFilterB(DbContext DB, Memo oSearchEntity, IQueryable<Memo> query, string sOperator = null)
        {
            if (oSearchEntity != null)
            {
                if (oSearchEntity.Id != 0) { query = query.Where(x => x.Id == oSearchEntity.Id); }
                ...
                if (oSearchEntity.TmodifyTime != null) { query = query.Where(x => x.TmodifyTime == oSearchEntity.TmodifyTime); }
                if (!string.IsNullOrWhiteSpace(oSearchEntity.Scontent)) { query = query.Where(x => x.Scontent == oSearchEntity.Scontent); }
            }
            return query;
        }
        protected override IQueryable<Memo> OrderBySingleField(IQueryable<Memo> query, string sSortName = null, string sSortOrder = null)
        {
            if (sSortOrder == "desc")
            {
                switch (sSortName)
                {
                    ...
                    case "Scontent": return query.OrderByDescending(x => x.Scontent).ThenBy(x => x.Id);
            ...
            else
            {
                switch (sSortName)
                {
                    ...
                    case "Scontent": return query.OrderBy(x => x.Scontent).ThenBy(x => x.Id);
