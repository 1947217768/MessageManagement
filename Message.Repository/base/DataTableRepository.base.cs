...
{
    public sealed partial class DataTableRepository : MessageManagementDBRepository<DataTable>, IDataTableRepository
    {
        protected override IQueryable<DataTable> SearchFilterB(DbContext DB, DataTable oSearchEntity, IQueryable<DataTable> query, string sOperator = null)
        {
            if (oSearchEntity != null)
            {
                if (oSearchEntity.Id != 0) { query = query.Where(x => x.Id == oSearchEntity.Id); }
                if (oSearchEntity.IdataBaseId != 0) { query = query.Where(x => x.IdataBaseId == oSearchEntity.IdataBaseId); }
                if (!string.IsNullOrWhiteSpace(oSearchEntity.StableName)) { query = query.Where(x => x.StableName.Contains(oSearchEntity.StableName)); }
        ...
        protected override IQueryable<DataTable> SelectFilterB(DbContext DB, DataTable oSearchEntity, IQueryable<DataTable> query, string sOperator = null)
        {
            if (oSearchEntity != null)
            {
                if (oSearchEntity.Id != 0) { query = query.Where(x => x.Id == oSearchEntity.Id); }
        ...
        protected override IQueryable<DataTable> OrderBySingleField(IQueryable<DataTable> query, string sSortName = null, string sSortOrder = null)
        {
            if (sSortOrder == "desc")
            {
                switch (sSortName)
                {
                    ...
                    case "StableName": return query.OrderByDescending(x => x.StableName).ThenBy(x => x.Id);
            ...
            else
            {
                switch (sSortName)
                {
                    ...
                    case "StableName": return query.OrderBy(x => x.StableName).ThenBy(x => x.Id);
